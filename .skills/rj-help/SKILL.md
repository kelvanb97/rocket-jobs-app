...
name: rj-help
description: >
    Use when the user says "/rj-help", "$rj-help", "rj-help", "what rj skills
    exist", "list rocket jobs skills", or "how do I use rocket jobs". Renders
    a table of all rj-* project skills with descriptions, prerequisites, and
    usage so the user can discover what's available in this repo.
user-invocable: true
---
...
Each `rj-*` skill can be invoked from any [agentskills.io](https://agentskills.io)-compatible harness. The invocation prefix depends on the harness:

- **Claude Code** (and other `/`-prefix harnesses): `/rj-install`, `/rj-setup`, …
- **Codex** (and other `$`-prefix harnesses): `$rj-install`, `$rj-setup`, …
- **Any harness**: describe the task in natural language (e.g. "install rocket jobs") and the assistant will pick the right skill.

The table below refers to skills by name (e.g. `rj-install`). Substitute the prefix your harness uses.

## Rocket Jobs Skills

| Skill | Claude Code | Codex | What it does | You'll need | When to use it |
| --- | --- | --- | --- | --- | --- |
| `rj-install` | `/rj-install` | `$rj-install` | Handles installing and updating the Rocket Jobs app on your computer. Works on Mac, Linux, and Windows. | Your computer and this folder open in an AI assistant such as Claude Code or Codex. | The first time you set up the app, or if you ever need a fresh reinstall. |
